...
export async function POST(req: Request, { params }: { params: { id: string } }) {
  ...
  const { data: event } = await supabase
    .from('events')
    .select('booth_rate_cents, contract_template_profile')
    .eq('id', contract.event_id)
    .maybeSingle<Pick<Event, 'booth_rate_cents' | 'contract_template_profile'>>();

  if (!requiresDiscountApproval(contract, event ?? undefined)) {
  ...
  await supabase.from('audit_log').insert({
    ...
    metadata: {
      ...
      booth_count: updated.booth_count,
      reason,
    },
  });
