-- Structured exhibitor address (canonical naming: line1 / line2)
-- 1) Move legacy single-line address into exhibitor_address_line1
...
begin
  if exists (
    ...
    if not exists (
      ...
      where table_schema = 'public'
        and table_name = 'contracts'
        and column_name = 'exhibitor_address_line1'
    ) then
      alter table contracts rename column exhibitor_address to exhibitor_address_line1;
    else
      execute $q$
        update contracts
        set exhibitor_address_line1 = coalesce(nullif(trim(exhibitor_address_line1), ''), exhibitor_address)
...
-- If underscore variants exist (from prior attempt), normalize them.
do $$
begin
  if exists (
    select 1 from information_schema.columns
    where table_schema = 'public' and table_name = 'contracts' and column_name = 'exhibitor_address_line_1'
  ) and not exists (
    select 1 from information_schema.columns
    where table_schema = 'public' and table_name = 'contracts' and column_name = 'exhibitor_address_line1'
  ) then
    alter table contracts rename column exhibitor_address_line_1 to exhibitor_address_line1;
  end if;

  if exists (
    select 1 from information_schema.columns
    where table_schema = 'public' and table_name = 'contracts' and column_name = 'exhibitor_address_line_2'
  ) and not exists (
    select 1 from information_schema.columns
    where table_schema = 'public' and table_name = 'contracts' and column_name = 'exhibitor_address_line2'
  ) then
    alter table contracts rename column exhibitor_address_line_2 to exhibitor_address_line2;
  end if;
end $$;
...
begin
  if exists (
    ...
    execute $q$
      update contracts
      set exhibitor_address_line1 = coalesce(nullif(trim(exhibitor_address_line1), ''), exhibitor_address_line1)
  ...
  if exists (
    ...
    execute $q$
      update contracts
      set exhibitor_address_line2 = coalesce(nullif(trim(exhibitor_address_line2), ''), exhibitor_address_line2)
...
with parsed as (
  ...
  from contracts
  where exhibitor_address_line1 is not null
    and trim(exhibitor_address_line1) <> ''
...
update contracts c
set
  exhibitor_address_line1 = nullif(trim(parsed.m[1]), ''),
