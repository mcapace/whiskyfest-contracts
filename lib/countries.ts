...
function parseCsvLine(line: string): { name: string; code: string } {
  let i = 0;
  const cols: string[] = [];
  while (i < line.length) {
    if (line[i] === '"') {
  ...
  return { name: cols[0] ?? '', code: cols[1] ?? '' };
}

const parsed = COUNTRIES_CSV.split('\n')
  .slice(1)
  .map((l) => parseCsvLine(l.trim()))
  .filter((c) => c.name && c.code)
  .map((c) => ({ code: c.code, name: c.name }));

const pinnedCodes = ['US', 'CA', 'GB'];
const pinned = pinnedCodes
  .map((code) => parsed.find((c) => c.code === code))
